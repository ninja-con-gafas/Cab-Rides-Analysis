...
# setup Apache Hadoop Distributed File System (HDFS) directories to import click stream data

hdfs dfs -rm -r /user/livy
hdfs dfs -mkdir -p /user/livy/click_stream_data

# run the Apache Spark Structured Streaming application to read data from Apache Kafka topic and store as CSV files in
# HDFS

spark-submit --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.4.5 data_sourcing/import_streaming_data.py

# download and install MySQL connector in Amazon Elastic Map Reduce (EMR) master node
...
# run an Apache Sqoop job to import bookings data from Amazon Relational Database Service (RDS) into HDFS
...
# setup feeds path and copy data into the feeds path for ingestion of the imported data into Apache Hive
...
# ingest the click stream data and bookings data into Hive tables

hive -f data_sourcing/ingest_streaming_data.sql
hive -f data_sourcing/ingest_batch_data.sql

# run Apache Spark application to aggregate bookings data, load the aggregated data into a Hive table and store as CSV
# files in HDFS
